...
// Formulario que aparece en el POS cuando no hay turno abierto.
// El empleado selecciona el turno (Mañana/Tarde/Noche), ingresa el
// efectivo inicial y abre el turno para empezar a vender.
// ════════════════════════════════════════════════════════════════════════════
...
interface AbrirTurnoProps {
  onAbierto: () => void;  // se llama cuando el turno se abre exitosamente
}
...
export const AbrirTurno: React.FC<AbrirTurnoProps> = ({ onAbierto }) => {
  ...
  // ── Cargar los turnos disponibles (Mañana, Tarde, Noche) ─────────────────
  ...
  const isValid = turnoSeleccionado !== '' && efectivoInicial !== '' &&
    !isNaN(parseFloat(efectivoInicial)) && parseFloat(efectivoInicial) >= 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onAbierto(); // notifica al padre → recarga → muestra el POS
    } catch (err: any) {
      ...
      <div className="w-full max-w-md px-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-neutral-500 text-sm">
            Para empezar a vender, abrí el turno del día
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-neutral-100 p-6">
          ...
          {/* Error */}
          {error && (
            <div className="mb-4 flex items-center gap-2 p-3 bg-danger-50 border border-danger-100
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1.5">
                <Clock size={14} className="inline mr-1.5 text-neutral-400" />
                Turno
              </label>
              {loadingTurnos ? (
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1.5">
                <DollarSign size={14} className="inline mr-1.5 text-neutral-400" />
              ...
              <p className="text-xs text-neutral-400 mt-1">
                Contá el dinero en caja antes de abrir
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Ej: falta cambio chico, hay descuento en gaseosas..."
                rows={2}
